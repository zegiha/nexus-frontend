...
function transformToArticleEntity(apiArticle: ArticleSummaryResponse): articleEntity {
  return {
    id: apiArticle.uuid,
...
// 전체 뉴스 조회 (페이지네이션 지원)
export async function getArticles(page: number = 1, limit: number = 20): Promise<articleEntity[]> {
  try {
...
export async function getAllCategoryHeadlines(): Promise<articleEntity[]> {
  try {
    ...
    response.forEach(categoryData => {
      if (categoryData && categoryData.articles && Array.isArray(categoryData.articles)) {
...
// 전체 언론사의 기사 조회
export async function getAllPressArticles(): Promise<{ [pressName: string]: articleEntity[] }> {
  try {
    ...
    response.forEach(pressData => {
      if (pressData && pressData.company && pressData.articles) {
        pressByArticles[pressData.company.name] = pressData.articles.map(transformToArticleEntity);
      }
    });
...
// 뉴스 검색
export async function searchArticles(query: string, page: number = 1, limit: number = 20): Promise<articleEntity[]> {
  try {
    const response = await apiClient<PaginatedArticleResponse>({
      method: 'GET',
      url: '/article/search',
...
// 뉴스 상세 조회
export async function getArticleById(uuid: string): Promise<articleEntity | null> {
  try {
...
export async function getArticlesByCategory(category: string, page: number = 1, limit: number = 20): Promise<articleEntity[]> {
  // 전체 헤드라인에서 특정 카테고리 필터링
  try {
    const allHeadlines = await getAllCategoryHeadlines();
    return allHeadlines.filter(article => article.category === category).slice(0, limit);
  } catch (error) {
